...
import { crudListCustomComponentAbove } from './config/crud-list-custom-component-above';
import { MatSnackBar } from '@angular/material/snack-bar';

@Component({
  selector: 'app-crud-list-custom-component-above',
  templateUrl: './crud-list-custom-component-above.component.html',
  styleUrls: ['./crud-list-custom-component-above.component.scss'],
  encapsulation: ViewEncapsulation.None
})
export class CrudListCustomComponentAboveComponent extends CrudListComponentInterface implements OnInit {
  //  Common Config
  identifier: string;
  header: CrudHeader;
  ...
  setCommonConfig(): void {
    super.setCommonConfigUsingCrud(crudListCustomComponentAbove);
    this.setOptions();
  }
  ...
  setFormConfig(userId: string): void {
    //  Config
    this.form = crudListCustomComponentAbove.form;
    this.actions = crudListCustomComponentAbove.actions;
    this.actionPages = crudListCustomComponentAbove.actionPages;

    //  Tab wise data/setting
    this.configFormData = {
      pageBackRoute: ["/example", "component", "crud", "crud-list-custom-component-above"],
  ...
  setFormsData(userId: string): any {
    if (userId == "new") {
      ...
      this.setTabDisplayMode(crudListCustomComponentAbove.form.tabs, FormDiaplyMode.ADD);
      CrudUtils.changeButtonLabelIcon(crudListCustomComponentAbove.form.tabs, "updateUser", "Save", "add");
      ...
      if (this.action == "editUser") {
        ...
        this.setTabDisplayMode(crudListCustomComponentAbove.form.tabs, FormDiaplyMode.EDIT);
      } else {
        this.displayMode = FormDiaplyMode.VIEW;
        this.setTabDisplayMode(crudListCustomComponentAbove.form.tabs, FormDiaplyMode.VIEW);
      }
      CrudUtils.changeButtonLabelIcon(crudListCustomComponentAbove.form.tabs, "updateUser", "Update", "edit");
  ...
  setListConfig(): void {
    super.setListConfigUsingCrud(crudListCustomComponentAbove);
  }
  ...
  buttonClick(action: Action) {
    ...
    switch (action.sourceIdentifier) {
      case "crudListCustomComponentAbove":
        this.actionCrudListCustomComponentAbove(action);
        break;
      case "userForm":
  ...
  actionCrudListCustomComponentAbove(action: Action) {
    switch (action.action) {
      case "addUser":
        this.router.navigate(['/example', 'component', 'crud', 'crud-list-custom-component-above', 'new'], { queryParams: { action: action.action }, skipLocationChange: false, replaceUrl: false });
        break;
        case "addAddress":
          this.actionAddAddress(action);
          break;
        case "goToGithub":
        window.open("https://github.com/khaitan-nitin/ngx-material-widget-example/tree/main/src/app/example/component/crud/crud-list-custom-component-above", "_blank");
        break;
    }
  }
  ...
  performBackToList(action: Action) {
    this.router.navigate(['/example/component/crud/crud-list-custom-component-above']);
  }

  actionUserList(action: Action) {
    switch (action.action) {
      case "editUser":
        this.router.navigate(['/example', 'component', 'crud', 'crud-list-custom-component-above', action.originalData['id']], { queryParams: { action: action.action }, skipLocationChange: false, replaceUrl: false });
        break;
      case "deleteUser":
