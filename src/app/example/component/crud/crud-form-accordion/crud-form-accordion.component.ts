...
import { crudFormInAccordion } from './config/crud-form-accordion';
import { MatSnackBar } from '@angular/material/snack-bar';

@Component({
  selector: 'app-crud-form-accordion',
  templateUrl: './crud-form-accordion.component.html',
  styleUrls: ['./crud-form-accordion.component.scss'],
  encapsulation: ViewEncapsulation.None
})
export class CrudFormAccordionComponent extends CrudListComponentInterface implements OnInit {
  //  Common Config
  identifier: string;
  header: CrudHeader;
  ...
  setCommonConfig(): void {
    super.setCommonConfigUsingCrud(crudFormInAccordion);
    this.setOptions();
  }
  ...
  setFormConfig(userId: string): void {
    //  Config
    this.form = crudFormInAccordion.form;
    this.actions = crudFormInAccordion.actions;
    this.actionPages = crudFormInAccordion.actionPages;

    //  Tab wise data/setting
    this.configFormData = {
      pageBackRoute: ["/example", "component", "crud", "crud-form-accordion"],
  ...
  setFormsData(userId: string): any {
    if (userId == "new") {
      ...
      this.setTabDisplayMode(crudFormInAccordion.form.tabs, FormDiaplyMode.ADD);
      CrudUtils.changeButtonLabelIcon(crudFormInAccordion.form.tabs, "updateUser", "Save", "add");
      ...
      if (this.action == "editUser") {
        ...
        this.setTabDisplayMode(crudFormInAccordion.form.tabs, FormDiaplyMode.EDIT);
      } else {
        this.displayMode = FormDiaplyMode.VIEW;
        this.setTabDisplayMode(crudFormInAccordion.form.tabs, FormDiaplyMode.VIEW);
      }
      CrudUtils.changeButtonLabelIcon(crudFormInAccordion.form.tabs, "updateUser", "Update", "edit");
  ...
  setListConfig(): void {
    super.setListConfigUsingCrud(crudFormInAccordion);
  }
  ...
  buttonClick(action: Action) {
    ...
    switch (action.sourceIdentifier) {
      case "crudFormInAccordion":
        this.actionCrudFormInAccordion(action);
        break;
      case "userForm":
  ...
  actionCrudFormInAccordion(action: Action) {
    switch (action.action) {
      case "addUser":
        this.router.navigate(['/example', 'component', 'crud', 'crud-form-accordion', 'new'], { queryParams: { action: action.action }, skipLocationChange: false, replaceUrl: false });
        break;
      case "addAddress":
        this.actionAddAddress(action);
        break;
      case "goToGithub":
        window.open("https://github.com/khaitan-nitin/ngx-material-widget-example/tree/main/src/app/example/component/crud/crud-form-accordion", "_blank");
        break;
    }
  }
  ...
  performBackToList(action: Action) {
    this.router.navigate(['/example/component/crud/crud-form-accordion']);
  }

  actionUserList(action: Action) {
    switch (action.action) {
      case "editUser":
        this.router.navigate(['/example', 'component', 'crud', 'crud-form-accordion', action.originalData['id']], { queryParams: { action: action.action }, skipLocationChange: false, replaceUrl: false });
        break;
      case "deleteUser":
