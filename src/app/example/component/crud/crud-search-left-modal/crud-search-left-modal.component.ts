...
import { searchLeftModalCrud } from './config/crud-search-left-modal';
import { MatSnackBar } from '@angular/material/snack-bar';

@Component({
  selector: 'app-crud-search-left-modal',
  templateUrl: './crud-search-left-modal.component.html',
  styleUrls: ['./crud-search-left-modal.component.scss'],
  encapsulation: ViewEncapsulation.None
})
export class CrudSearchLeftModalComponent extends CrudListComponentInterface implements OnInit {
  //  Common Config
  identifier: string;
  header: CrudHeader;
  ...
  setCommonConfig(): void {
    super.setCommonConfigUsingCrud(searchLeftModalCrud);
    this.setOptions();
  }
  ...
  setFormConfig(userId: string): void {
    //  Config
    this.form = searchLeftModalCrud.form;
    this.actions = searchLeftModalCrud.actions;
    this.actionPages = searchLeftModalCrud.actionPages;

    //  Tab wise data/setting
    this.configFormData = {
      pageBackRoute: ["/example", "component", "crud", "crud-search-left-modal"],
  ...
  setFormsData(userId: string): any {
    if (userId == "new") {
      ...
      this.setTabDisplayMode(searchLeftModalCrud.form.tabs, FormDiaplyMode.ADD);
      CrudUtils.changeButtonLabelIcon(searchLeftModalCrud.form.tabs, "updateUser", "Save", "add");
      ...
      if (this.action == "editUser") {
        ...
        this.setTabDisplayMode(searchLeftModalCrud.form.tabs, FormDiaplyMode.EDIT);
      } else {
        this.displayMode = FormDiaplyMode.VIEW;
        this.setTabDisplayMode(searchLeftModalCrud.form.tabs, FormDiaplyMode.VIEW);
      }
      CrudUtils.changeButtonLabelIcon(searchLeftModalCrud.form.tabs, "updateUser", "Update", "edit");
  ...
  setListConfig(): void {
    super.setListConfigUsingCrud(searchLeftModalCrud);
  }
  ...
  actionBasicCrud(action: Action) {
    switch (action.action) {
      case "addUser":
        this.router.navigate(['/example', 'component', 'crud', 'crud-search-left-modal', 'new'], { queryParams: { action: action.action }, skipLocationChange: false, replaceUrl: false });
        break;
        case "addAddress":
          this.actionAddAddress(action);
          break;
        case "goToGithub":
        window.open("https://github.com/khaitan-nitin/ngx-material-widget-example/tree/main/src/app/example/component/crud/crud-search-left-modal", "_blank");
        break;
    }
  }
  ...
  performBackToList(action: Action) {
    this.router.navigate(['/example/component/crud/crud-search-left-modal']);
  }

  actionUserList(action: Action) {
    switch (action.action) {
      case "editUser":
        this.router.navigate(['/example', 'component', 'crud', 'crud-search-left-modal', action.originalData['id']], { queryParams: { action: action.action }, skipLocationChange: false, replaceUrl: false });
        break;
      case "deleteUser":
