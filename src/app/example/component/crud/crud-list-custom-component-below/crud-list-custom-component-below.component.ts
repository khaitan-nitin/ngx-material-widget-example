...
import { crudListCustomComponentBelow } from './config/crud-list-custom-component-below';
import { MatSnackBar } from '@angular/material/snack-bar';

@Component({
  selector: 'app-crud-list-custom-component-below',
  templateUrl: './crud-list-custom-component-below.component.html',
  styleUrls: ['./crud-list-custom-component-below.component.scss'],
  encapsulation: ViewEncapsulation.None
})
export class CrudListCustomComponentBelowComponent extends CrudListComponentInterface implements OnInit {
  //  Common Config
  identifier: string;
  header: CrudHeader;
  ...
  setCommonConfig(): void {
    super.setCommonConfigUsingCrud(crudListCustomComponentBelow);
    this.setOptions();
  }
  ...
  setFormConfig(userId: string): void {
    //  Config
    this.form = crudListCustomComponentBelow.form;
    this.actions = crudListCustomComponentBelow.actions;
    this.actionPages = crudListCustomComponentBelow.actionPages;

    //  Tab wise data/setting
    this.configFormData = {
      pageBackRoute: ["/example", "component", "crud", "crud-list-custom-component-below"],
  ...
  setFormsData(userId: string): any {
    if (userId == "new") {
      ...
      this.setTabDisplayMode(crudListCustomComponentBelow.form.tabs, FormDiaplyMode.ADD);
      CrudUtils.changeButtonLabelIcon(crudListCustomComponentBelow.form.tabs, "updateUser", "Save", "add");
      ...
      if (this.action == "editUser") {
        ...
        this.setTabDisplayMode(crudListCustomComponentBelow.form.tabs, FormDiaplyMode.EDIT);
      } else {
        this.displayMode = FormDiaplyMode.VIEW;
        this.setTabDisplayMode(crudListCustomComponentBelow.form.tabs, FormDiaplyMode.VIEW);
      }
      CrudUtils.changeButtonLabelIcon(crudListCustomComponentBelow.form.tabs, "updateUser", "Update", "edit");
  ...
  setListConfig(): void {
    super.setListConfigUsingCrud(crudListCustomComponentBelow);
  }
  ...
  buttonClick(action: Action) {
    ...
    switch (action.sourceIdentifier) {
      case "crudListCustomComponentBelow":
        this.actionCrudListCustomComponentBelow(action);
        break;
      case "userForm":
  ...
  actionCrudListCustomComponentBelow(action: Action) {
    switch (action.action) {
      case "addUser":
        this.router.navigate(['/example', 'component', 'crud', 'crud-list-custom-component-below', 'new'], { queryParams: { action: action.action }, skipLocationChange: false, replaceUrl: false });
        break;
        case "addAddress":
          this.actionAddAddress(action);
          break;
        case "goToGithub":
        window.open("https://github.com/khaitan-nitin/ngx-material-widget-example/tree/main/src/app/example/component/crud/crud-list-custom-component-below", "_blank");
        break;
    }
  }
  ...
  performBackToList(action: Action) {
    this.router.navigate(['/example/component/crud/crud-list-custom-component-below']);
  }

  actionUserList(action: Action) {
    switch (action.action) {
      case "editUser":
        this.router.navigate(['/example', 'component', 'crud', 'crud-list-custom-component-below', action.originalData['id']], { queryParams: { action: action.action }, skipLocationChange: false, replaceUrl: false });
        break;
      case "deleteUser":
