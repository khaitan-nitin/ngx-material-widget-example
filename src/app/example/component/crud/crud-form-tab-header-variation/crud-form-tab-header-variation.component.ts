...
import { crudFormTabHeaderVariation } from './config/crud-form-tab-header-variation';
import { MatSnackBar } from '@angular/material/snack-bar';

@Component({
  selector: 'app-crud-form-tab-header-variation',
  templateUrl: './crud-form-tab-header-variation.component.html',
  styleUrls: ['./crud-form-tab-header-variation.component.scss'],
  encapsulation: ViewEncapsulation.None
})
export class CrudFormTabHeaderVariationComponent extends CrudListComponentInterface implements OnInit {
  //  Common Config
  identifier: string;
  header: CrudHeader;
  ...
  setCommonConfig(): void {
    super.setCommonConfigUsingCrud(crudFormTabHeaderVariation);
    this.setOptions();
  }
  ...
  setFormConfig(userId: string): void {
    //  Config
    this.form = crudFormTabHeaderVariation.form;
    this.actions = crudFormTabHeaderVariation.actions;
    this.actionPages = crudFormTabHeaderVariation.actionPages;

    //  Tab wise data/setting
    this.configFormData = {
      pageBackRoute: ["/example", "component", "crud", "crud-form-tab-header-variation"],
  ...
  setFormsData(userId: string): any {
    if (userId == "new") {
      ...
      this.setTabDisplayMode(crudFormTabHeaderVariation.form.tabs, FormDiaplyMode.ADD);
      CrudUtils.changeButtonLabelIcon(crudFormTabHeaderVariation.form.tabs, "updateUser", "Save", "add");
      ...
      if (this.action == "editUser") {
        ...
        this.setTabDisplayMode(crudFormTabHeaderVariation.form.tabs, FormDiaplyMode.EDIT);
      } else {
        this.displayMode = FormDiaplyMode.VIEW;
        this.setTabDisplayMode(crudFormTabHeaderVariation.form.tabs, FormDiaplyMode.VIEW);
      }
      CrudUtils.changeButtonLabelIcon(crudFormTabHeaderVariation.form.tabs, "updateUser", "Update", "edit");
  ...
  setListConfig(): void {
    super.setListConfigUsingCrud(crudFormTabHeaderVariation);
  }
  ...
  buttonClick(action: Action) {
    ...
    switch (action.sourceIdentifier) {
      case "crudFormTabHeaderVariation":
        this.actionCrudFormTabHeaderVariation(action);
        break;
      case "userForm":
  ...
  actionCrudFormTabHeaderVariation(action: Action) {
    switch (action.action) {
      case "addUser":
        this.router.navigate(['/example', 'component', 'crud', 'crud-form-tab-header-variation', 'new'], { queryParams: { action: action.action }, skipLocationChange: false, replaceUrl: false });
        break;
        case "addAddress":
          this.actionAddAddress(action);
          break;
        case "goToGithub":
        window.open("https://github.com/khaitan-nitin/ngx-material-widget-example/tree/main/src/app/example/component/crud/crud-form-tab-header-variation", "_blank");
        break;
    }
  }
  ...
  performBackToList(action: Action) {
    this.router.navigate(['/example/component/crud/crud-form-tab-header-variation']);
  }

  actionUserList(action: Action) {
    switch (action.action) {
      case "editUser":
        this.router.navigate(['/example', 'component', 'crud', 'crud-form-tab-header-variation', action.originalData['id']], { queryParams: { action: action.action }, skipLocationChange: false, replaceUrl: false });
        break;
      case "deleteUser":
