...
import { crudFormCustomComponentAbove } from './config/crud-form-custom-component-above';
import { MatSnackBar } from '@angular/material/snack-bar';

@Component({
  selector: 'app-crud-form-custom-component-above',
  templateUrl: './crud-form-custom-component-above.component.html',
  styleUrls: ['./crud-form-custom-component-above.component.scss'],
  encapsulation: ViewEncapsulation.None
})
export class CrudFormCustomComponentAboveComponent extends CrudListComponentInterface implements OnInit {
  //  Common Config
  identifier: string;
  header: CrudHeader;
  ...
  setCommonConfig(): void {
    super.setCommonConfigUsingCrud(crudFormCustomComponentAbove);
    this.setOptions();
  }
  ...
  setFormConfig(userId: string): void {
    //  Config
    this.form = crudFormCustomComponentAbove.form;
    this.actions = crudFormCustomComponentAbove.actions;
    this.actionPages = crudFormCustomComponentAbove.actionPages;

    //  Tab wise data/setting
    this.configFormData = {
      pageBackRoute: ["/example", "component", "crud", "crud-form-custom-component-above"],
  ...
  setFormsData(userId: string): any {
    if (userId == "new") {
      ...
      this.setTabDisplayMode(crudFormCustomComponentAbove.form.tabs, FormDiaplyMode.ADD);
      CrudUtils.changeButtonLabelIcon(crudFormCustomComponentAbove.form.tabs, "updateUser", "Save", "add");
      ...
      if (this.action == "editUser") {
        ...
        this.setTabDisplayMode(crudFormCustomComponentAbove.form.tabs, FormDiaplyMode.EDIT);
      } else {
        this.displayMode = FormDiaplyMode.VIEW;
        this.setTabDisplayMode(crudFormCustomComponentAbove.form.tabs, FormDiaplyMode.VIEW);
      }
      CrudUtils.changeButtonLabelIcon(crudFormCustomComponentAbove.form.tabs, "updateUser", "Update", "edit");
  ...
  setListConfig(): void {
    super.setListConfigUsingCrud(crudFormCustomComponentAbove);
  }
  ...
  buttonClick(action: Action) {
    ...
    switch (action.sourceIdentifier) {
      case "crudFormCustomComponentAbove":
        this.actionCrudFormCustomComponentAbove(action);
        break;
      case "userForm":
  ...
  actionCrudFormCustomComponentAbove(action: Action) {
    switch (action.action) {
      case "addUser":
        this.router.navigate(['/example', 'component', 'crud', 'crud-form-custom-component-above', 'new'], { queryParams: { action: action.action }, skipLocationChange: false, replaceUrl: false });
        break;
        case "addAddress":
          this.actionAddAddress(action);
          break;
        case "goToGithub":
        window.open("https://github.com/khaitan-nitin/ngx-material-widget-example/tree/main/src/app/example/component/crud/crud-form-custom-component-above", "_blank");
        break;
    }
  }
  ...
  performBackToList(action: Action) {
    this.router.navigate(['/example/component/crud/crud-form-custom-component-above']);
  }

  actionUserList(action: Action) {
    switch (action.action) {
      case "editUser":
        this.router.navigate(['/example', 'component', 'crud', 'crud-form-custom-component-above', action.originalData['id']], { queryParams: { action: action.action }, skipLocationChange: false, replaceUrl: false });
        break;
      case "deleteUser":
