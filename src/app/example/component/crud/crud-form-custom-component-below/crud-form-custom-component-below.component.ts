...
import { crudFormCustomComponentBelow } from './config/crud-form-custom-component-below';
import { MatSnackBar } from '@angular/material/snack-bar';

@Component({
  selector: 'app-crud-form-custom-component-below',
  templateUrl: './crud-form-custom-component-below.component.html',
  styleUrls: ['./crud-form-custom-component-below.component.scss'],
  encapsulation: ViewEncapsulation.None
})
export class CrudFormCustomComponentBelowComponent extends CrudListComponentInterface implements OnInit {
  //  Common Config
  identifier: string;
  header: CrudHeader;
  ...
  setCommonConfig(): void {
    super.setCommonConfigUsingCrud(crudFormCustomComponentBelow);
    this.setOptions();
  }
  ...
  setFormConfig(userId: string): void {
    //  Config
    this.form = crudFormCustomComponentBelow.form;
    this.actions = crudFormCustomComponentBelow.actions;
    this.actionPages = crudFormCustomComponentBelow.actionPages;

    //  Tab wise data/setting
    this.configFormData = {
      pageBackRoute: ["/example", "component", "crud", "crud-form-custom-component-below"],
  ...
  setFormsData(userId: string): any {
    if (userId == "new") {
      ...
      this.setTabDisplayMode(crudFormCustomComponentBelow.form.tabs, FormDiaplyMode.ADD);
      CrudUtils.changeButtonLabelIcon(crudFormCustomComponentBelow.form.tabs, "updateUser", "Save", "add");
      ...
      if (this.action == "editUser") {
        ...
        this.setTabDisplayMode(crudFormCustomComponentBelow.form.tabs, FormDiaplyMode.EDIT);
      } else {
        this.displayMode = FormDiaplyMode.VIEW;
        this.setTabDisplayMode(crudFormCustomComponentBelow.form.tabs, FormDiaplyMode.VIEW);
      }
      CrudUtils.changeButtonLabelIcon(crudFormCustomComponentBelow.form.tabs, "updateUser", "Update", "edit");
  ...
  setListConfig(): void {
    super.setListConfigUsingCrud(crudFormCustomComponentBelow);
  }
  ...
  buttonClick(action: Action) {
    ...
    switch (action.sourceIdentifier) {
      case "crudFormCustomComponentBelow":
        this.actionCrudFormCustomComponentBelow(action);
        break;
      case "userForm":
  ...
  actionCrudFormCustomComponentBelow(action: Action) {
    switch (action.action) {
      case "addUser":
        this.router.navigate(['/example', 'component', 'crud', 'crud-form-custom-component-below', 'new'], { queryParams: { action: action.action }, skipLocationChange: false, replaceUrl: false });
        break;
        case "addAddress":
          this.actionAddAddress(action);
          break;
        case "goToGithub":
        window.open("https://github.com/khaitan-nitin/ngx-material-widget-example/tree/main/src/app/example/component/crud/crud-form-custom-component-below", "_blank");
        break;
    }
  }
  ...
  performBackToList(action: Action) {
    this.router.navigate(['/example/component/crud/crud-form-custom-component-below']);
  }

  actionUserList(action: Action) {
    switch (action.action) {
      case "editUser":
        this.router.navigate(['/example', 'component', 'crud', 'crud-form-custom-component-below', action.originalData['id']], { queryParams: { action: action.action }, skipLocationChange: false, replaceUrl: false });
        break;
      case "deleteUser":
