...
import { searchRightModalCrud } from './config/crud-search-right-modal';
import { MatSnackBar } from '@angular/material/snack-bar';

@Component({
  selector: 'app-crud-search-right-modal',
  templateUrl: './crud-search-right-modal.component.html',
  styleUrls: ['./crud-search-right-modal.component.scss'],
  encapsulation: ViewEncapsulation.None
})
export class CrudSearchRightModalComponent extends CrudListComponentInterface implements OnInit {
  //  Common Config
  identifier: string;
  header: CrudHeader;
  ...
  setCommonConfig(): void {
    super.setCommonConfigUsingCrud(searchRightModalCrud);
    this.setOptions();
  }
  ...
  setFormConfig(userId: string): void {
    //  Config
    this.form = searchRightModalCrud.form;
    this.actions = searchRightModalCrud.actions;
    this.actionPages = searchRightModalCrud.actionPages;

    //  Tab wise data/setting
    this.configFormData = {
      pageBackRoute: ["/example", "component", "crud", "crud-search-right-modal"],
  ...
  setFormsData(userId: string): any {
    if (userId == "new") {
      ...
      this.setTabDisplayMode(searchRightModalCrud.form.tabs, FormDiaplyMode.ADD);
      CrudUtils.changeButtonLabelIcon(searchRightModalCrud.form.tabs, "updateUser", "Save", "add");
      ...
      if (this.action == "editUser") {
        ...
        this.setTabDisplayMode(searchRightModalCrud.form.tabs, FormDiaplyMode.EDIT);
      } else {
        this.displayMode = FormDiaplyMode.VIEW;
        this.setTabDisplayMode(searchRightModalCrud.form.tabs, FormDiaplyMode.VIEW);
      }
      CrudUtils.changeButtonLabelIcon(searchRightModalCrud.form.tabs, "updateUser", "Update", "edit");
  ...
  setListConfig(): void {
    super.setListConfigUsingCrud(searchRightModalCrud);
  }
  ...
  actionBasicCrud(action: Action) {
    switch (action.action) {
      case "addUser":
        this.router.navigate(['/example', 'component', 'crud', 'crud-search-right-modal', 'new'], { queryParams: { action: action.action }, skipLocationChange: false, replaceUrl: false });
        break;
        case "addAddress":
          this.actionAddAddress(action);
          break;
        case "goToGithub":
        window.open("https://github.com/khaitan-nitin/ngx-material-widget-example/tree/main/src/app/example/component/crud/crud-search-right-modal", "_blank");
        break;
    }
  }
  ...
  performBackToList(action: Action) {
    this.router.navigate(['/example/component/crud/crud-search-right-modal']);
  }

  actionUserList(action: Action) {
    switch (action.action) {
      case "editUser":
        this.router.navigate(['/example', 'component', 'crud', 'crud-search-right-modal', action.originalData['id']], { queryParams: { action: action.action }, skipLocationChange: false, replaceUrl: false });
        break;
      case "deleteUser":
