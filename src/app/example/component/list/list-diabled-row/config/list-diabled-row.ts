...
export const listDiabledRow: List = {
    identifier: "listDiabledRow",
    header: {
        title: "Disable row",
        icon: {
            font: "play_disabled"
    ...
    description: {
        text: "Conditionally disable row and its action button(s). i.e - we have disabled row where user have gmail id",
        bgColor: "#e6ebf9"
    },
    shade: {
    ...
    disableRow: (row: User) => {
        ...
        if (row.email.indexOf("gmail.com") > -1)    {
            isDisable = true;
        }
        return isDisable;
    }
