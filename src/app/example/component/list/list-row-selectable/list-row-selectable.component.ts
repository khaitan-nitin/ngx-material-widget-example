...
import { listRowSelectable } from './config/list-row-selectable';

@Component({
  selector: 'app-list-row-selectable',
  templateUrl: './list-row-selectable.component.html',
  styleUrls: ['./list-row-selectable.component.scss']
})
export class ListRowSelectableComponent implements OnInit {
  listRowSelectable: List;
  parent: ObjectTree;
  listReset: boolean;
  ...
  ngOnInit(): void {
    this.listRowSelectable = listRowSelectable;
  ...
  buttonClick(buttonAction: Action) {
    ...
    if (buttonAction.action == "goToGithub") {
      window.open("https://github.com/khaitan-nitin/ngx-material-widget-example/tree/main/src/app/example/component/list/list-row-selectable", "_blank");
    }
  }
