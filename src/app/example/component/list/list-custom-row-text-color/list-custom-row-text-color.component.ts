...
import { listCustomRowTextColor } from './config/list-custom-row-text-color';

@Component({
  selector: 'app-list-custom-row-text-color',
  templateUrl: './list-custom-row-text-color.component.html',
  styleUrls: ['./list-custom-row-text-color.component.scss']
})
export class ListCustomRowTextColorComponent implements OnInit {
  listCustomRowTextColor: List;
  parent: ObjectTree;
  listReset: boolean;
  ...
  ngOnInit(): void {
    this.listCustomRowTextColor = listCustomRowTextColor;
  ...
  buttonClick(buttonAction: Action) {
    ...
    if (buttonAction.action == "goToGithub") {
      window.open("https://github.com/khaitan-nitin/ngx-material-widget-example/tree/main/src/app/example/component/list/list-custom-row-text-color", "_blank");
    }
  }
