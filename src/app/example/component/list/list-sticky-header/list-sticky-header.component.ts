...
import { listStickyHeader } from './config/list-sticky-header';

@Component({
  selector: 'app-list-sticky-header',
  templateUrl: './list-sticky-header.component.html',
  styleUrls: ['./list-sticky-header.component.scss']
})
export class ListStickyHeaderComponent implements OnInit {
  listStickyHeader: List;
  parent: ObjectTree;
  listReset: boolean;
  ...
  ngOnInit(): void {
    this.listStickyHeader = listStickyHeader;
  ...
  buttonClick(buttonAction: Action) {
    ...
    if (buttonAction.action == "goToGithub") {
      window.open("https://github.com/khaitan-nitin/ngx-material-widget-example/tree/main/src/app/example/component/list/list-sticky-header", "_blank");
    }
  }
