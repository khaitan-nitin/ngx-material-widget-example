...
import { listWithNestedList } from './config/list-with-nested-list';

@Component({
  selector: 'app-list-with-nested-list',
  templateUrl: './list-with-nested-list.component.html',
  styleUrls: ['./list-with-nested-list.component.scss']
})
export class ListWithNestedListComponent implements OnInit {
  listWithNestedList: List;
  parent: ObjectTree;
  listReset: boolean;
  ...
  ngOnInit(): void {
    this.listWithNestedList = listWithNestedList;
  ...
  buttonClick(buttonAction: Action) {
    ...
    if (buttonAction.action == "goToGithub") {
      window.open("https://github.com/khaitan-nitin/ngx-material-widget-example/tree/main/src/app/example/component/list/list-with-nested-list", "_blank");
    }
  }
