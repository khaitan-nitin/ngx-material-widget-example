...
import { listWithColumnSelector } from './config/list-with-column-selector';

@Component({
  selector: 'app-list-with-column-selector',
  templateUrl: './list-with-column-selector.component.html',
  styleUrls: ['./list-with-column-selector.component.scss']
})
export class ListWithColumnSelectorComponent implements OnInit {
  listWithColumnSelector: List;
  parent: ObjectTree;
  listReset: boolean;
  ...
  ngOnInit(): void {
    this.listWithColumnSelector = listWithColumnSelector;
  ...
  buttonClick(buttonAction: Action) {
    ...
    if (buttonAction.action == "goToGithub") {
      window.open("https://github.com/khaitan-nitin/ngx-material-widget-example/tree/main/src/app/example/component/list/list-with-column-selector", "_blank");
    }
  }
